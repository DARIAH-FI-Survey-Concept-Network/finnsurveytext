---
title: "Tutorial1-Prepare and prepare_svydesign"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Tutorial1-Prepare_and_prepare_svydesign}
...
3. `fst_rm_stop_punct()`
4. `fst_prepare()`
5. `fst_format_svydesign()`
6. `fst_prepare_svydesign()`
...
The `finnsurveytext` package uses the CoNLL-U format. This tutorial demonstrates the process of preparing Finnish survey text data into this format using functions in r/01_prepare.r and r/01b_prepare_svydesign.r.
...
## The Whole Story - data in dataframe
If your data is in a dataframe, a single function, `fst_prepare()` (which calls all the data preparation functions within the package) can be used to prepare the data into the required CoNNL-U format. 
...
* `data` is the dataframe of interest. In this case, we are using data that comes with the package called 'child'. Otherwise, if you read in a csv containing a dataframe, such as through `read.csv()` in base R for use in this tutorial.
...
* `data` is the dataframe of interest. In this case, we are using data that comes with the package called 'dev_coop'. Otherwise, if you read in a csv containing a dataframe, such as through `read.csv()` in base R for use in this tutorial.
...
## The Whole Story - data in svydesign object

If instead, your survey data is in a dataframe, the corresponding function is `fst_prepare_svydesign()`
...
```{r}
conllu_dev_q11_1 <- fst_format(data = dev_coop, 
                               question = "q11_1", 
                               id = "fsd_id",)
conllu_cb_bullying <- fst_format(data = child, 
                                 question = "q7", 
                                 id = "fsd_id",
                                 model = "tdt")
```
