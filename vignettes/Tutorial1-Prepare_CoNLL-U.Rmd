...
## Introduction
This tutorial demonstrates the process of preparing Finnish survey text data into the CoNLL-U format required for the `finnsurveytext` package functions using functions in r/0.1_prepare_conll-u.r.
...
Both of these will be demonstrated below but either can be used to complete the tutorial. You should download one of these in csv format and save it in a location that you can easily find. 

## The Whole Story
A single function, `fst_prepare_connl` (which calls all the data preparation functions within the package) can be used to prepare the data into the required CoNNL-U format. 

#### 1. Bullying Data
Using our bullying data, we can call this function as follows: 

```{r eval = FALSE}
prepd_bullying <- fst_prepare_connlu(data = bullying_data, field = "q7", stopword_list = "nltk")
```
...
* We have chosen to remove stopwords from the "nltk" stopword list in this example. To find the relevant lists of Finnish stopwords, you can run the `fst_rm_stopwords` function which is outlined below. 
...
```{r eval = FALSE}
prepd_lonely <- fst_prepare_connlu(lonely_data, field = "q9", stopword_list = "none", model = "tdt")
```
...
* We have chosen *not* to remove stopwords in this example. 
...
Stopword lists are lists of common words (like "and", "the",  and "is", or in Finnish "olla", "ollet", "ollen", and "on"...) which are often filtered out of the data, leaving less frequently-occurring, and thus more more meaningful, words remaining. The
...
* trims trailing whitespace from the data; and, 
* removes NA values. 
...
#### Remove stopwords from CoNLL-U data
`fst_rm_stopwords`

This (optional) function will remove stopwords from the CoNLL-U data. `fst_find_stopwords` can be used to find options for stopwords lists. 

`fst_rm_stopwords` takes 2 arguments: 
...
```{r}
conllu_lonely_nltk <- fst_rm_stopwords(data = conllu_lonely)
