...
To use these functions, we run the preparation functions which are in r/01_prepare_conll-u.R (and which are covered in detail in Tutorial1-Prepare CoNLL-U) for each group separately. It is expected that you may choose to split the data using a different question within the survey such as a categorical question (eg. gender or level of education) or an ordinal variable (such as aged or income). In this tutorial, we will look at comparing responses to a question based on gender. 
