...
lonely_concepts_full <- fst_cn_search(conllu_lonely, 'yksinäisyys, tunne, tuntea')
```
...
```{r}
bullying_edges <-fst_cn_edges(conllu_bullying_iso, 'kiusata, lyöminen, lyödä, potkia')
lonely_edges <- fst_cn_edges(conllu_lonely_nltk, 'yksinäisyys, tunne, tuntea', threshold = 3)
lonely_edges_full <- fst_cn_edges(conllu_lonely, concept = 'yksinäisyys, tunne, tuntea', threshold=5)
```
...
lonely_nodes_full <- fst_cn_nodes(conllu_lonely, lonely_edges_full)
```
