...
* We can choose whether to include or exclude surveys with no response in our splitting comlumn by setting `exclude_nulls`. The default value is FALSE.
...
```{r}
fst_comparison_cloud(df, 'gender', use_column_weights = TRUE)
fst_comparison_cloud(df, 'region', use_column_weights = TRUE)

```

Finally we have the comparison **concept network**:
